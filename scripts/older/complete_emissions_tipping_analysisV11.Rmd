...
packages <- c("tidyverse", "lubridate", "plotly", "DT", "kableExtra", "viridis", "patchwork", "scales", "ggrepel")
f.ipak(packages)
...
# BATTERY CLIMATE VALUE WITH COMPLETE SUPPLY CHAIN - CORRECTED STRATEGY
calculate_battery_complete_value <- function() {
  
  # CORRECTED: Battery charges during peak solar, discharges during evening peak
  # This extends methane avoidance beyond direct solar hours
  
  # Charging during peak solar (12-2 PM) - storing excess clean solar
  charge_hour_emissions <- calculate_hourly_grid_emissions(13) # 1 PM - midday solar peak
  
  # Discharging during evening peak (6-8 PM) - when solar declining but gas peakers still running  
  discharge_hour_emissions <- calculate_hourly_grid_emissions(19) # 7 PM - evening peak
  ...
  # CORRECTED CALCULATION: Battery extends solar's methane avoidance into evening hours
  # The "avoided" emissions are from NOT using grid power during evening gas peaker dispatch
  # Instead using stored solar energy from midday
  
  # Evening emissions avoided by using stored solar instead of grid
  daily_avoided_lbs <- battery_capacity * discharge_hour_emissions * round_trip_efficiency
  annual_avoided_lbs <- daily_avoided_lbs * 365
  annual_avoided_tons <- annual_avoided_lbs / 2000
  
  # The charging doesn't add emissions because it uses excess solar (not grid power)
  # This is the key difference from generic load-shifting analysis
  
  return(list(
    charge_hour_emissions = charge_hour_emissions,  # Not used (solar charging)
    discharge_hour_emissions = discharge_hour_emissions, # Avoided by battery
    annual_co2_avoided_tons = annual_avoided_tons,
    battery_capacity_kwh = battery_capacity,
    strategy = "solar_charge_evening_discharge",
    description = "Battery charged with excess solar during midday, discharged during evening gas peaker hours"
  ))
}
...
# Visualize battery operation - CORRECTED STRATEGY
p_battery_operation <- ggplot(battery_schedule, aes(x = hour)) +
  ...
  scale_fill_manual(values = c("Base Consumption" = "gray",
                              "charge" = "orange", "discharge" = "green", "idle" = "transparent"),
                    name = "Battery Action") +
  scale_color_manual(values = c("Grid Emissions" = "red"), name = "") +
  labs(title = "Solar+Battery Strategy: Charge from Excess Solar, Discharge During Evening Peak",
       subtitle = paste("Extends methane avoidance by", round(battery_complete_value$annual_co2_avoided_tons, 2), 
                       "tons CO2e/year into evening hours (6-10 PM)"),
       x = "Hour of Day",
       y = "Power (kW) / Emissions Factor (scaled)",
       caption = "Orange: Charging from excess solar (12-2 PM), Green: Discharging during evening gas peakers (6-10 PM)") +
  theme_minimal() +
  theme(legend.position = "bottom")

print(p_battery_operation)

cat("\n=== CORRECTED BATTERY ANALYSIS WITH SOLAR CHARGING ===\n")
cat("ACTUAL BATTERY STRATEGY:\n")
cat("- Charge: 12-2 PM during peak solar (excess solar energy)\n")
cat("- Discharge: 6-10 PM during evening peak (gas peakers still running)\n")
cat("- Effect: Extends clean solar energy into evening gas peaker hours\n\n")

cat("Evening gas peaker emissions (6-10 PM):", round(battery_complete_value$discharge_hour_emissions, 3), "lbs CO2e/kWh\n")
cat("Battery capacity:", battery_complete_value$battery_capacity_kwh, "kWh\n")
cat("Annual methane avoided by battery:", round(battery_complete_value$annual_co2_avoided_tons, 2), "tons CO2e/year\n")

cat("\nKEY INSIGHT: Battery extends solar's methane avoidance beyond direct generation hours\n")
cat("Total system (solar + battery) avoids gas peakers during 12-2 PM and 6-10 PM daily\n")
cat("This creates a 6-hour window of methane avoidance from GA Power gas turbines\n")
```
