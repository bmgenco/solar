---
title: "House Component Analysis - Solar & Energy Decision Framework v9"
...
data_dir <- "data"  # Data is in solar/data, not ../data
...
## 1a. Emporia Circuit-Level Data (MODIFIED - Multiple Resolutions, Keep All Variables)

```{r emporia_data_import}
# Function to load Emporia circuit-level energy monitoring data at multiple resolutions
# EDIT: Load all resolutions, keep ALL circuit data, allow resolution selection
load_emporia_data <- function(resolution = "day_1", files_list = emporia_files) {
  ...
  # Parse datetime - try multiple formats for robustness
  datetime_formats <- c("%Y-%m-%d %H:%M:%S", "%Y-%m-%d", "%m/%d/%Y %H:%M", "%m/%d/%Y")
...
[Rest of sections 1b through 3 remain the same as previous version...]
...
# anomaly_events <- detect_data_anomalies(emporia_daily, ecobee_data)
```
...
# wh_analysis <- analyze_water_heater_circuit(emporia_daily, function_variables$water_heater_circuit)
```
...
cat("=== HOUSE COMPONENT USAGE INSTRUCTIONS V9 ===\n\n")

cat("1. CONFIGURATION:\n")
cat("   - Review and update function_variables parameters\n")
cat("   - File paths are now configured with actual data filenames\n")
...
cat("   - Multiple Emporia resolutions available (1SEC through 1DAY)\n")
cat("   - Use emporia_daily for system sizing, emporia_1min for LRA analysis\n\n")

cat("3. KEY CHANGES IN V9:\n")
cat("   - Fixed caching issues with correct file paths\n")
cat("   - Multi-resolution Emporia data loading\n")
cat("   - dirname() working directory approach\n")
cat("   - All variables preserved from input data\n")
cat("   - LRA and battery optimization capability\n\n")

cat("4. MULTI-RESOLUTION ANALYSIS:\n")
cat("   - 1SEC/1MIN: LRA detection, battery power sizing\n")
cat("   - 15MIN/1H: Solar matching, time-of-use optimization\n")
cat("   - 1DAY: Annual energy balance, system sizing\n\n")

cat("5. SYSTEM SIZING:\n")
cat("   - Run solar system sizing with consumption baseline\n")
cat("   - Battery analysis with high-resolution data capability\n")
cat("   - Heat pump conversion load calculations\n\n")

cat("6. EXPORT FOR OTHER COMPONENTS:\n")
cat("   - Execute compile_house_profile() to create house_profile.RData\n")
cat("   - Includes all analysis results and function_variables used\n\n")

cat("OUTPUT: Complete house_profile.RData for financial and emissions components\n")
```


# 4. Graphing and Basic analysis
## 4a. Emporia usage data
 -graphing and summary stats
```{r}
# Daily Energy Usage Analysis with ggplot2
library(ggplot2)
library(dplyr)
library(scales)
library(lubridate)
...
emporia_plot_data <- emporia_daily %>%
  mutate(
    # Combine AC and fridge usage
    ac_fridge_kwh = `Air_Conditioner_17_19__AC_kwh` + `Fridge_Freezer_10__Fridge_and_outlets_kwh`,
  ...
  filter(!is.na(total_house_kwh), !is.na(ac_fridge_kwh))

# Create the plot
daily_usage_plot <- ggplot(emporia_plot_data, aes(x = datetime)) +
  geom_line(aes(y = total_house_kwh, color = "Total House Usage"), 
            size = 0.8, alpha = 0.8) +
  geom_line(aes(y = ac_fridge_kwh, color = "AC + Fridge"), 
            size = 0.7, alpha = 0.9) +
  
  # Formatting
  scale_color_manual(
    values = c("Total House Usage" = "#2E86AB", "AC + Fridge" = "#F24236"),
    name = "Usage Type"
  ) +
  
  # X-axis formatting - monthly major breaks
  scale_x_datetime(
  ...
  scale_y_continuous(
    ...
    labels = comma_format(suffix = " kWh")
  ) +
  
  # Labels and theme
  labs(
    title = "Daily Energy Usage - Total vs AC+Fridge",
    subtitle = "2265 Ridgedale Road, Decatur GA",
    x = "Date",
    y = "Daily Energy Usage (kWh)",
    caption = paste("Data range:", min(emporia_plot_data$datetime), "to", max(emporia_plot_data$datetime))
  ) +
...
# Display the plot
print(daily_usage_plot)

# Summary statistics for peak usage analysis
cat("=== DAILY USAGE SUMMARY STATISTICS ===\n")

usage_stats <- emporia_plot_data %>%
  summarise(
    # Total usage stats
    total_mean = mean(total_house_kwh, na.rm = TRUE),
    total_median = median(total_house_kwh, na.rm = TRUE),
    total_max = max(total_house_kwh, na.rm = TRUE),
    total_min = min(total_house_kwh, na.rm = TRUE),
    
    # AC + Fridge stats
    ac_fridge_mean = mean(ac_fridge_kwh, na.rm = TRUE),
    ac_fridge_median = median(ac_fridge_kwh, na.rm = TRUE),
    ac_fridge_max = max(ac_fridge_kwh, na.rm = TRUE),
    
    # Percentage calculations
    ac_fridge_pct_of_total = (mean(ac_fridge_kwh, na.rm = TRUE) / mean(total_house_kwh, na.rm = TRUE)) * 100,
    
    # Date range
    date_range_days = as.numeric(difftime(max(datetime), min(datetime), units = "days"))
  )

print(usage_stats)

# Peak usage days identification
peak_days <- emporia_plot_data %>%
  arrange(desc(total_house_kwh)) %>%
  slice_head(n = 10) %>%
  select(datetime, total_house_kwh, ac_fridge_kwh, month_name, week_day) %>%
  mutate(
    ac_fridge_pct = round((ac_fridge_kwh / total_house_kwh) * 100, 1)
  )
...
monthly_stats <- emporia_plot_data %>%
  group_by(month_name) %>%
  summarise(
    days = n(),
    avg_total = round(mean(total_house_kwh, na.rm = TRUE), 2),
    avg_ac_fridge = round(mean(ac_fridge_kwh, na.rm = TRUE), 2),
    ac_fridge_pct = round((mean(ac_fridge_kwh, na.rm = TRUE) / mean(total_house_kwh, na.rm = TRUE)) * 100, 1),
    max_day = round(max(total_house_kwh, na.rm = TRUE), 2),
    .groups = 'drop'
  ) %>%
...
monthly_stats <- monthly_stats %>%
  rename(
    "Month" = month_name,
    "Daily Average kWh" = avg_total,
    "Daily Average of Fridge and AC" = avg_ac_fridge,
    "AC & Fridge %" = ac_fridge_pct,
    "Peak Day kWh" = max_day
  )
...
day_type_stats <- emporia_plot_data %>%
  mutate(day_type = if_else(week_day %in% c("Sat", "Sun"), "Weekend", "Weekday")) %>%
  group_by(day_type) %>%
  summarise(
    avg_total = round(mean(total_house_kwh, na.rm = TRUE), 2),
    avg_ac_fridge = round(mean(ac_fridge_kwh, na.rm = TRUE), 2),
...
```
 - export tables
```{r}
# TABLE EXPORT METHODS FOR R SUMMARY STATISTICS
library(knitr)
library(kableExtra)
library(writexl)
library(readr)

# === METHOD 1: CSV EXPORT (Best for data sharing) ===
# Export usage_stats to CSV
write_csv(usage_stats, "output/daily_usage_summary.csv")
write_csv(peak_days, "output/peak_usage_days.csv")
write_csv(monthly_stats, "output/monthly_usage_patterns.csv")
write_csv(day_type_stats, "output/weekday_weekend_comparison.csv")

cat("CSV files exported to output/ directory\n")

# === METHOD 2: EXCEL EXPORT (Best for formatted sharing) ===
# Create list of tables for multi-sheet Excel file
table_list <- list(
  "Summary_Stats" = usage_stats,
  "Peak_Days" = peak_days,
  "Monthly_Patterns" = monthly_stats,
  "Day_Type_Comparison" = day_type_stats
)

# Export to Excel with multiple sheets
write_xlsx(table_list, "output/energy_usage_analysis.xlsx")

cat("Excel file with multiple sheets exported: energy_usage_analysis.xlsx\n")

# === METHOD 3: FORMATTED CONSOLE OUTPUT (Copy-paste friendly) ===
# Better formatted output for copying
cat("\n=== COPY-PASTE FRIENDLY SUMMARY ===\n")
print(kable(usage_stats, 
            digits = 2, 
            col.names = c("Total Mean", "Total Median", "Total Max", "Total Min",
                         "AC+Fridge Mean", "AC+Fridge Median", "AC+Fridge Max", 
                         "AC+Fridge %", "Days in Dataset"),
            caption = "Daily Energy Usage Summary Statistics"))

# === METHOD 4: HTML TABLE (For reports/emails) ===
# Create formatted HTML table
html_table <- usage_stats %>%
  kable(digits = 2, 
        col.names = c("Total Mean (kWh)", "Total Median (kWh)", "Total Max (kWh)", 
                     "Total Min (kWh)", "AC+Fridge Mean (kWh)", "AC+Fridge Median (kWh)", 
                     "AC+Fridge Max (kWh)", "AC+Fridge % of Total", "Dataset Days"),
        caption = "Daily Energy Usage Summary - Ridgedale Road") %>%
...
# Save HTML table
writeLines(as.character(html_table), "output/usage_summary_table.html")

# === METHOD 5: TRANSPOSED TABLE (Easier to read) ===
# Transpose for better readability
usage_stats_transposed <- usage_stats %>%
  gather(key = "Metric", value = "Value") %>%
  mutate(
    Value = round(Value, 2),
    Metric = case_when(
      Metric == "total_mean" ~ "Average Daily Usage (kWh)",
      Metric == "total_median" ~ "Median Daily Usage (kWh)", 
      Metric == "total_max" ~ "Peak Daily Usage (kWh)",
      Metric == "total_min" ~ "Minimum Daily Usage (kWh)",
      Metric == "ac_fridge_mean" ~ "Average AC+Fridge (kWh)",
      Metric == "ac_fridge_median" ~ "Median AC+Fridge (kWh)",
      Metric == "ac_fridge_max" ~ "Peak AC+Fridge (kWh)",
      Metric == "ac_fridge_pct_of_total" ~ "AC+Fridge % of Total",
      Metric == "date_range_days" ~ "Days in Dataset",
      TRUE ~ Metric
    )
  )

cat("\n=== TRANSPOSED SUMMARY TABLE ===\n")
print(kable(usage_stats_transposed, 
            col.names = c("Energy Metric", "Value"),
            caption = "Energy Usage Analysis Summary"))

# Export transposed version
write_csv(usage_stats_transposed, "output/usage_summary_transposed.csv")

# === METHOD 6: FORMATTED TEXT SUMMARY (For reports) ===
cat("\n=== FORMATTED TEXT SUMMARY ===\n")
cat("ENERGY USAGE ANALYSIS SUMMARY\n")
cat("=============================\n")
cat(sprintf("Average daily usage: %.1f kWh\n", usage_stats$total_mean))
cat(sprintf("Peak daily usage: %.1f kWh\n", usage_stats$total_max))
cat(sprintf("AC + Fridge average: %.1f kWh (%.1f%% of total)\n", 
            usage_stats$ac_fridge_mean, usage_stats$ac_fridge_pct_of_total))
cat(sprintf("Analysis period: %d days\n", usage_stats$date_range_days))

# === METHOD 7: CREATE ALL EXPORTS WITH ONE FUNCTION ===
export_all_tables <- function() {
  ...
  # CSV exports
  write_csv(usage_stats, "output/01_daily_usage_summary.csv")
  write_csv(peak_days, "output/02_peak_usage_days.csv") 
  write_csv(monthly_stats, "output/03_monthly_usage_patterns.csv")
  write_csv(day_type_stats, "output/04_weekday_weekend_comparison.csv")
  
  # Excel export (multi-sheet)
  table_list <- list(
    "Daily_Summary" = usage_stats,
    "Peak_Days" = peak_days,
    "Monthly_Patterns" = monthly_stats,
    "Weekend_vs_Weekday" = day_type_stats
  )
  write_xlsx(table_list, "output/energy_usage_complete_analysis.xlsx")
  
  # Summary report
  cat("All tables exported successfully!\n")
  cat("Files created:\n")
  cat("- CSV files: 01-04_*.csv\n") 
  cat("- Excel file: energy_usage_complete_analysis.xlsx\n")
  cat("- Location: output/ directory\n")
}

# Run the export function
export_all_tables()

# === QUICK SHARING FORMAT ===
cat("\n=== QUICK SHARE FORMAT ===\n")
cat("Daily Usage: Avg=", round(usage_stats$total_mean,1), "kWh, Peak=", round(usage_stats$total_max,1), "kWh\n")
cat("AC+Fridge: Avg=", round(usage_stats$ac_fridge_mean,1), "kWh (", round(usage_stats$ac_fridge_pct_of_total,1), "% of total)\n")
```
