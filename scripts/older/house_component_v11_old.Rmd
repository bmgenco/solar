---
title: "House Component Analysis - Solar & Energy Decision Framework v11"
...
<!--
SOLAR & ENERGY EFFICIENCY DECISION ANALYSIS - THREE COMPONENT FRAMEWORK v11
================================================================================
...
CRITICAL UPDATES v11:
- CORRECTED gas appliance allocation: 175/310/15 therms (water/HVAC/stove)
...
### GAS APPLIANCE ALLOCATION - CORRECTED v11 ####
function_variables$gas_usage <- list(
  total_annual_therms = 500,
  # CORRECTED ALLOCATION based on seasonal bill analysis
  stove_therms = 15,        # 3% - year-round cooking (minimal)
  water_heater_therms = 175, # 35% - constant baseline load
  hvac_therms = 310,        # 62% - winter heating only
  
  # Validation data from actual bills
  summer_baseline = 22,      # therms/month (Jun-Sep avg) = stove + water heater
  winter_peak = 70,          # therms/month (Dec-Feb avg) = all appliances
  ...
  allocation_method = "seasonal_bill_analysis",
  validation_note = "Summer baseline validates water_heater + stove = 190 therms"
)
...
cat("Function variables initialized for v11. USER ACTIONS REQUIRED:\n")
...
cat("4. CORRECTED: Gas allocation now uses 175/310/15 therms\n")
```
...
## 1c. Gas Usage Data with CORRECTED Appliance Allocation
```{r}
---
title: "House Component Analysis - Solar & Energy Decision Framework v11"
subtitle: "2265 Ridgedale Road, Decatur GA - Data Processing & System Sizing"
...
```{r gas_usage_corrected}
# Load and process gas usage data with CORRECTED appliance breakdown
load_gas_usage_data <- function(file_path = gas_usage_file) {
  ...
  cat("=== LOADING GAS USAGE DATA WITH CORRECTED ALLOCATION ===\n")
  ...
  # Calculate usage baselines for emissions analysis - CORRECTED v11
  ...
  gas_data <- gas_data %>%
    mutate(
      # CORRECTED v11: Estimate water heating + stove vs space heating split
  ...
  # DATA-DERIVED ALLOCATION - Calculate from actual bills + Ecobee runtime
  ...
  cat("=== GAS USAGE ANALYSIS FROM BILL DATA ===\n")
...
## 1d. Ecobee-Gas Integration Analysis
...
integrate_ecobee_gas_analysis <- function(ecobee_data, gas_analysis) {
  ...
  # Define HVAC failure period from manual events
  hvac_failure_dates <- function_variables$manual_events %>%
    filter(event_type == "hvac_failure") %>%
    pull(date)
  
  # Calculate monthly heating runtime from Ecobee data
  ecobee_monthly <- ecobee_data %>%
    filter(!is.na(heat_runtime_min)) %>%
    mutate(
      year_month = format(datetime, "%Y-%m"),
      month = month(datetime),
      date = as.Date(datetime),
      # Flag HVAC failure period
      is_hvac_failure_period = date %in% hvac_failure_dates
    ) %>%
  ...
  # Calculate annual heating runtime and efficiency (excluding failure periods)
  annual_heating_stats <- integrated_data %>%
    filter(is_heating_month, !is.na(total_heat_runtime_hours_normal), normal_heating_days > 0) %>%
    summarise(
      total_annual_heating_hours_normal = sum(total_heat_runtime_hours_normal),
  ...
  # Analyze HVAC failure period specifically
  ...
  cat("ECOBEE-GAS INTEGRATION RESULTS:\n")
  cat("Total annual heating runtime (normal):", round(annual_heating_stats$total_annual_heating_hours_normal, 0), "hours\n")
  cat("Estimated heating gas usage:", round(annual_heating_stats$total_estimated_heating_therms, 1), "therms\n")
  cat("Average efficiency (normal periods):", round(annual_heating_stats$avg_therms_per_hour_normal, 3), "therms/hour\n")
  cat("HVAC failure impact:", annual_heating_stats$hvac_failure_impact_days, "days excluded from analysis\n\n")
  ...
  cat("\nVALIDATION AGAINST BILL-DERIVED HVAC USAGE:\n")
  cat("Bill-derived HVAC:", round(gas_analysis$baseline_usage$hvac_heating_annual, 1), "therms\n")
  cat("Ecobee-validated HVAC:", round(annual_heating_stats$total_estimated_heating_therms, 1), "therms\n")
  cat("Difference:", round(abs(gas_analysis$baseline_usage$hvac_heating_annual - annual_heating_stats$total_estimated_heating_therms), 1), "therms\n")
  cat("Note: Analysis excludes HVAC failure period for accurate efficiency calculation\n")
  
  return(list(
    integrated_monthly = integrated_data,
    annual_stats = annual_heating_stats,
    hvac_failure_analysis = hvac_failure_analysis,
    heating_efficiency_therms_per_hour = annual_heating_stats$avg_therms_per_hour_normal
  ))
}

# Integrate Ecobee and gas data using processed data objects
...
## 1d. Georgia Power Data (Working Excel Loader)
...
compile_house_profile_v11 <- function(emporia_data = NULL, 
                                      ecobee_data = NULL,
                                      ga_power_data = NULL,
                                      gas_analysis = NULL) {
  
  cat("=== COMPILING HOUSE PROFILE v11 FOR EXPORT ===\n")
  
  # Baseline energy consumption with CORRECTED gas allocation
  baseline_consumption <- list(
    ...
    # CORRECTED gas appliance breakdown
    gas_appliance_breakdown = list(
      stove_therms = 15,        # 3%
      water_heater_therms = 175, # 35%  
      hvac_therms = 310,        # 62%
      allocation_method = "seasonal_bill_analysis"
    ),
  ...
  # Heat pump loads with CORRECTED estimates
  heat_pump_loads <- list(
    # Based on corrected gas allocation
    ...
    # Gas elimination by appliance
    hpwh_gas_eliminated = 175,  # therms (water heater only)
    hp_hvac_gas_eliminated = 310, # therms (space heating only)
    total_gas_eliminated = 485,   # therms (keep 15 for stove)
  ...
  # Compile complete house profile v11
  house_profile <- list(
    ...
    # Include gas analysis results if available
    gas_analysis_results = if(!is.null(gas_analysis)) gas_analysis else NULL
  )
  
  # Save to RData file for other components
  cat("Saving house profile v11 to house_profile.RData\n")
  save(house_profile, file = file.path(output_dir, "house_profile.RData"))
  
  cat("\n=== HOUSE PROFILE v11 SUMMARY ===\n")
  cat("CORRECTED GAS ALLOCATION:\n")
  cat("  Stove: 15 therms/year (3%)\n")
  cat("  Water Heater: 175 therms/year (35%)\n") 
  cat("  HVAC: 310 therms/year (62%)\n\n")
...
# Compile and export house profile v11 with corrected gas allocation
house_profile <- compile_house_profile_v11(emporia_daily, ecobee_data, ga_power_data, gas_analysis)

# Save to output directory
save(house_profile, file = "output/house_profile.RData")
```

# 3. Usage Instructions v11

```{r usage_instructions}
cat("=== HOUSE COMPONENT v11 USAGE INSTRUCTIONS ===\n\n")

cat("VERSION 11 UPDATES:\n")
cat("✓ CORRECTED gas appliance allocation: 175/310/15 therms\n")
...
cat("✓ All working data loaders from v10 maintained\n\n")

cat("GAS APPLIANCE ALLOCATION (CORRECTED):\n")
cat("  Stove: 15 therms/year (3%) - minimal cooking load\n")
cat("  Water Heater: 175 therms/year (35%) - constant baseline\n") 
cat("  HVAC: 310 therms/year (62%) - winter heating only\n")
cat("  Validation: Summer baseline = 22 therms/month ≈ stove + water heater\n\n")
...
if(exists("gas_analysis") && !is.null(gas_analysis)) {
  cat("✓ Gas usage data loaded with corrected allocation\n")
} else {
  cat("⚠ Gas usage data not loaded\n")
}

cat("\nNEXT STEPS:\n")
cat("1. Upload this v11 script to new chat\n")
cat("2. Run to generate updated house_profile.RData\n")
cat("3. Upload updated emissions analysis script\n")
cat("4. Run emissions analysis with corrected gas allocation\n")
cat("5. Financial component will automatically use corrected data\n\n")

cat("OUTPUT: house_profile.RData with v11 corrections ready for emissions component\n")
```
## 1d. Ecobee-Gas Integration Analysis
...
```{r ecobee_gas_integration}
# Function to integrate Ecobee heating runtime with gas usage for validation
integrate_ecobee_gas_analysis <- function(ecobee_data, gas_analysis) {
  ...
  # Get gas usage by month for correlation
  gas_monthly <- gas_analysis$gas_data %>%
    filter(!is.na(therms_used)) %>%
    mutate(year_month = format(bill_date, "%Y-%m")) %>%
    select(year_month, therms_used, avg_temperature, heating_degree_days)
...
# Integrate Ecobee and gas data if both available
if(exists("ecobee_data") && exists("gas_analysis")) {
  ecobee_gas_integration <- integrate_ecobee_gas_analysis(ecobee_data, gas_analysis)
} else {
  cat("Ecobee or gas data not loaded - skipping integration analysis\n")
