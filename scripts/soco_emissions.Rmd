...
pull_soco <- function(year, api_key) {
  ...
  for (i in seq_along(starts)) {
    ...
    chunks[[i]] <- eia_get(
      ...
      data = "value",
      frequency = "hourly",
      facets = list(respondent = "SOCO"),
      start = s, end = e, length = 5000
    )
  }
  
  do.call(rbind, chunks) %>%
    mutate(time = as.POSIXct(period, format = "%Y-%m-%dT%H", tz = "UTC")) %>%
    select(-period) %>%
...
# ============================================================================
```
add ing columns to datfaram


```{r}
add_time_cols <- function(df) {
  df %>%
    rename_with(~ gsub("-", "_", .x)) %>%   # type-name → type_name, etc.
    mutate(
      datetime_utc = time,
      datetime_est = time - hours(5),
      date     = as.Date(datetime_est),
      year     = year(datetime_est),
      month    = month(datetime_est),
      month_name = month(datetime_est, label = TRUE),
      hour_utc = hour(time),
      hour_est = hour(datetime_est),
      season   = case_when(
...
soco_2024<-add_time_cols(soco_2024)
soco_2025<-add_time_cols(soco_2025)
...
plot_soco <- function(df) {
  ...
  p1 <- ggplot(df %>% filter(value > 0), 
               aes(x = time, y = value, color = type_name)) +
    geom_line(alpha = 0.3) +
  ...
  p3 <- df %>%
    ...
    mutate(hour = hour_est) %>%
    group_by(hour, type_name) %>%
    summarise(avg_mwh = mean(value, na.rm = TRUE), .groups = "drop") %>%
    ggplot(aes(x = hour, y = avg_mwh, fill = type_name)) +
    geom_area() +
    scale_x_continuous(breaks = seq(0, 23, 3)) +
    scale_fill_viridis_d() +
    labs(title = paste("SOCO Average Hourly Generation Profile —", yr),
         subtitle = "Time of day (EST)",
         y = "Average MWh", x = "Hour of Day (EST)", fill = "Fuel") +
    theme_minimal(base_size = 13)
